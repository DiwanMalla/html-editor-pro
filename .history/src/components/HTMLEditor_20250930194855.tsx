...
${code.js}
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between p-6 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border-b border-slate-200/60 dark:border-slate-700/60">
        <div className="flex items-center space-x-6">
          <div className="flex space-x-1 bg-slate-100 dark:bg-slate-700 rounded-xl p-1.5 shadow-inner">
            {editorTabs.map((tab) => (
              <Button
                key={tab.id}
              ...
              >
                <div className={`${activeTab === tab.id ? 'text-violet-500' : ''}`}>
                  {tab.icon}
                </div>
                <span>{tab.label}</span>
              </Button>
