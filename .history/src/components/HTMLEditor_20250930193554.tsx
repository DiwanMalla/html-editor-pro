...
export default function HTMLEditor({ isFullscreen = false, onToggleFullscreen }: HTMLEditorProps) {
  ...
  const resetCode = () => {
    const confirmReset = window.confirm('Are you sure you want to reset all code? This action cannot be undone.')
    if (confirmReset) {
      const defaultCode = editorTabs.reduce((acc, tab) => ({
        ...acc,
...
${code.js}
  ...
  return (
    <div className={`flex flex-col h-full bg-gray-50 dark:bg-gray-900 transition-colors ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
      {/* Header */}
